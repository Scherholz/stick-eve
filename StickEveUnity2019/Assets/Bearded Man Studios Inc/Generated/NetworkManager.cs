...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] RealTimeCombatNetworkManagerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is RealTimeCombatNetworkManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (RealTimeCombatNetworkManagerNetworkObject.Length > 0 && RealTimeCombatNetworkManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(RealTimeCombatNetworkManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<RealTimeCombatNetworkManagerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateRealTimeCombatNetworkManager instead, its shorter and easier to type out ;)")]
		public RealTimeCombatNetworkManagerBehavior InstantiateRealTimeCombatNetworkManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RealTimeCombatNetworkManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<RealTimeCombatNetworkManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<RealTimeCombatNetworkManagerBehavior>().networkObject = (RealTimeCombatNetworkManagerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of RealTimeCombatNetworkManager
		/// </summary>
		/// <returns>
		/// A local instance of RealTimeCombatNetworkManagerBehavior
		/// </returns>
		/// <param name="index">The index of the RealTimeCombatNetworkManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public RealTimeCombatNetworkManagerBehavior InstantiateRealTimeCombatNetworkManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RealTimeCombatNetworkManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<RealTimeCombatNetworkManagerBehavior>();
